...
Store the HTML annotation data with the issue so a report can be reproduced without manually rediscovering the highlighted region. Bounds are percentages of that specific crop, not page coordinates:

```yaml
id: QA-01
status: fail
title: Search field has the wrong radius
evidence:
  expected_crop: crops/qa-01-expected.png
  actual_crop: crops/qa-01-actual.png
annotations:
  expected:
    - { left: 4.2, top: 67.8, width: 91.6, height: 14.1, label: Reference pill field }
  actual:
    - { left: 4.2, top: 67.8, width: 91.6, height: 14.1, label: Rounded-rectangle field }
```

Use separate expected and actual bounds whenever layout changes make a shared rectangle misleading. These annotations are presentation metadata for `qa-audit-template.html`; comparison records remain the evidence source.
